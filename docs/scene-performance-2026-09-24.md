...
3画面のCPU最適化を実装し、選曲バーの不要な生成、固定判定円グラフ、
Lua併用時の構造・グラフキャッシュ、Lua状態参照の負荷を削減した。
以下の前半は最適化前の調査結果。後半に実装とCPU・実画面のA/B計測を記録する。

現在使用中のスキンは選曲がECFN、単曲リザルトがStarseeker、コースリザルトがECFN。
変更後の実画面比較でもVSyncOffで約120 FPSのまま、FPSの上昇は確認できなかった。
描画計画作成は4条件すべてで短縮したが、surface取得に約6.6～7.5ms/フレームかかる。
取得・presentを除く処理全体では、単曲StarseekerとLuxez-Flatの短縮を確認した。
選曲と1曲コースの処理全体は実行間の揺れがあり、明確な改善とは判断しない。
...
これらの揺れは今回の変更前から存在する。
後続のplan dump調査では、mz-selectの同一バイナリ間の差は時計の秒表示1桁のUVだけだった。
ref 21～26は実際の日時を読むため、日時を含む画面は実行を分けるとhashが一致しない。
ECFNのスクロールとLua変更前後のmz-selectもhashが不一致だったため、全命令一致の確認対象には含めない。
代表フレームの詳細は `BMZ_SCENE_PROFILE_DUMP=<directory>` で保存できる。

### 2. 固定された判定円グラフの再利用

Starseekerの固定円グラフだけを再利用し、判定数・画像ソースの変更で破棄する。
timer、アニメーション、offset、hover、条件付き描画があるdestinationは対象外。
当初のsegmentごとに確認する案は284.6→296.0µsと悪化したため不採用とし、
判定数・ソースの確認をフレーム単位へ移した。
修正版は5回の交互計測で287.5→266.9µs、7.1%短縮（既定設定）。
この比較には後述のcache leaseも含まれるが、StarseekerリザルトにはLua callbackがなく、
従来からキャッシュ経路を通るため、主な変更は円グラフの再利用となる。
判定数・ソース差し替えと動的segmentについて、非キャッシュ経路との比較テストが成功した。

### 3. Luaと描画キャッシュの併用

描画中はキャッシュを一時的に所有し、mutexを解放するleaseを導入した。
Luaがあっても選曲の構造・静止画像と、リザルトの構造・グラフを再利用する。
Playも同じleaseを使い、callback実行中にはキャッシュlockを保持しない。
再入時は空のキャッシュを使い、panicで巻き戻っても所有していたキャッシュを返す。
callback自体は従来と同じ順序・回数で実行する。

| 条件 | 変更前（µs） | 変更後（µs） | 短縮 |
|---|---:|---:|---:|
| 選曲 ECFN・既定 | 308.8 | 300.6 | 2.7% |
| 選曲 mz-select・既定 | 406.0 | 398.8 | 1.8% |
| 単曲 ECFN・既定 | 38.9 | 35.5 | 8.7% |
| 単曲 Luxez-Flat・既定 | 199.8 | 185.9 | 6.9% |
| コース ECFN・既定4曲 | 43.1 | 36.9 | 14.3% |
| コース Luxez-Flat・既定4曲 | 191.0 | 181.4 | 5.0% |
| コース ECFN・現在設定10曲 | 188.2 | 83.9 | 55.4% |

表は円グラフ初案入りのバイナリ同士でleaseの有無だけを比較したもの。
両側の円グラフ条件をそろえて変更を切り分けている。
WMIIコースは90.9→82.0µsだったが、起動間で平均commandsも233.2→234.0と変化しており参考値。
選曲・リザルトのcallback順序と動的値、Lua併用時のグラフ再利用と新規リザルトでの無効化、
再入・unwindを検証。変更後のskin描画テスト416件が成功した。

### 4. Lua状態参照の受け渡し

callbackごとのdispatcherを、文字列キーのregistryから非公開テーブルの数値slotへ移した。
`main_state.number` などの呼び出しごとの文字列検索と、登録・解除の負荷を減らす。
借用stateを持つ関数は引き続きLua scopeで管理し、正常終了・エラー・unwindのいずれでも解除する。
関数そのもののフレーム全体での共有は行わず、callback順序や命令数制限を維持した。

| 条件・既定設定 | 変更前（µs） | 変更後（µs） | 短縮 |
|---|---:|---:|---:|
| 単曲 Luxez-Flat | 182.5 | 171.4 | 6.0% |
| コース Luxez-Flat・4曲 | 177.5 | 168.1 | 5.3% |
| 単曲 MILLIONDOLLAR | 369.5 | 362.8 | 1.8% |

mz-select選曲は388.3→386.1µs（0.6%）と小さく、主な効果とは扱わない。
WMIIコースは80.1→77.4µsだが、commandsの起動間差があるため参考値。
bmz-skinテストは214成功、調査時と同じWMII関連3件のみ失敗。
Luxez-Flat単曲・コース、MILLIONDOLLAR単曲は全348フレームの描画hashが一致した。
修正版の円グラフもStarseeker単曲で変更前後のhashが一致した。

### 全変更を通したCPU比較

調査時点のコードと全変更後を5回ずつ交互計測。現在のprofileのoptions/filesを使用した。

| 画面・スキン | 変更前（µs） | 変更後（µs） | 短縮 |
|---|---:|---:|---:|
| 選曲 ECFN | 319.4 | 300.6 | 5.9% |
| 単曲リザルト Starseeker | 319.8 | 283.6 | 11.3% |
...
実画面の初回A/B再計測はMacの画面ロックによりsurfaceを取得できず、最初のpresentも成功していなかった。
この試行で出た数千FPSは描画を完了しないループ頻度なので全て除外した。
ログは `scene-optimization/invalid-locked/` に分け、集計にもpresent成功とGPU encode/queue実行の確認を追加した。

### 全変更を通した実画面比較（9月24～25日）

ロック解除後、最適化前の `8788d87d` 相当のreleaseアプリと、
`33c96700` までの変更を含むreleaseアプリを比較した。
Metal / VSyncOff（requested・effectiveともImmediate）/ 2944×1656 / Native /
FPS制限0・バックグラウンド制限0。ビルド・テスト・他の計測とは同時実行しない。

- 4条件を変更前→後、後→前の順で各2回。有効な計測は合計16回。
  各回の終了直前の集計を除く120フレーム×6区間、各条件・各バイナリ1440フレームを採用した。
  表は2回の平均。FPSはログ時刻による描画開始頻度で、scanout頻度ではない。
- 専用のprofile・DBと32個の合成BMSを使用。選曲はルートの静止画面、
  単曲は `--boot-result-sample`、コースは112ノーツの譜面1曲を完走したCourseResult。
  コースはスキン読込後3秒以降の区間だけを集計した。
- ECFN選曲・Starseeker単曲・ECFNコースは現在のoptions/files/offsetsを適用。
  Luxez-Flat単曲は既定設定。コースのみ背景の `---AAA = "Random"` を計測用profileで
  `BG/AAA/S_BG_AAA.png` に固定した。普段使うprofile・DB・外部スキンは変更していない。
- 選曲の最初の比較では難易度表の初回取得が走り、前後で表示行が変わっていたため除外した。
  `auto_fetch_on_startup = false` でも未取得の表は取得されるため、
  再計測では既定27件すべてを計測用configに明記して `enabled = false` にした。
  背景がランダムだったコースも固定画像で取り直した。採用した全ログで難易度表取得は発生していない。

描画計画作成の平均時間（ms）:

| 画面・スキン | 変更前 | 変更後 | 短縮 |
|---|---:|---:|---:|
| 選曲 ECFN | 0.160 | 0.133 | 16.8% |
| 単曲リザルト Starseeker | 0.673 | 0.461 | 31.5% |
| 単曲リザルト Luxez-Flat | 0.840 | 0.702 | 16.5% |
| コースリザルト ECFN・1曲 | 0.313 | 0.292 | 6.5% |

実アプリ全体の平均時間（ms）。矢印は変更前→後。
「取得・present除外」はCPU側で測る実時間で、queue待ちなども含む。GPU実行時間ではない。

| 画面・スキン | 取得・present除外 | surface取得 | 全redraw | FPS |
|---|---:|---:|---:|---:|
| 選曲 ECFN | 0.758 → 0.776 | 7.509 → 7.485 | 8.273 → 8.267 | 120.0 → 120.0 |
| 単曲 Starseeker | 1.566 → 1.231 | 6.648 → 7.000 | 8.233 → 8.247 | 120.0 → 120.0 |
| 単曲 Luxez-Flat | 1.759 → 1.592 | 6.439 → 6.613 | 8.220 → 8.226 | 120.0 → 120.0 |
| コース ECFN・1曲 | 1.061 → 1.051 | 7.151 → 7.155 | 8.230 → 8.225 | 120.0 → 120.0 |

Starseekerは取得・present除外時間が21.4%、Luxez-Flatは9.5%短縮し、両順で改善した。
一方、選曲は動画更新時間が平均0.279→0.319msと揺れ、処理全体の改善は確認できない。
動画upload数は各720フレーム中360枚で両側とも同じ。コースも処理全体の差は
1巡目1.090→1.055ms、逆順1.031→1.046msと方向が変わり、平均0.9%を確実な改善とは扱わない。
CPU probeの4曲・10曲とこの1曲コースは別条件であり、改善率をそのまま適用しない。

全redrawの区間p99の最大値は、選曲9.020→8.851ms、Starseeker8.947→8.942ms、
Luxez-Flat9.157→9.234ms、コース9.038→9.096ms。
これは全1440フレームをまとめたp99ではなく、120フレーム区間ごとのp99の最大値。
FPSやフレーム時間の裾が一律に改善したという結果ではない。

代表スクリーンショットの前後を目視比較し、選曲の行・数値、リザルトの成績・グラフの欠落を認めなかった。
日時、動画背景、点滅・パーティクルの位相差があるため、画素単位の完全一致は主張しない。
描画commandsは選曲144、Starseeker1046、Luxez-Flat569、コース237で前後同数。
固定グラフのキャッシュ画像化によって矩形instance数が減るため、instance数と表示要素数は同一ではない。

採用データは `.local/performance/2026-09-24/scene-optimization/surface-valid-summary.json`。
単曲2種のログ・画像は同ディレクトリの `surface-{0,1}-{base,final}/surface/`、
選曲・コースの再計測分は `surface-controlled-{0,1}-{base,final}/surface/` に保存した。
`surface-fixed-*` の試行も難易度表取得の検出で失敗しており、採用データには含めていない。
