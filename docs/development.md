...
beatoraja の参照ソースは `.local/beatoraja/`、beatoraja 対応スキン例は `.local/skins/` に置かれます。これらは global gitignore 管理で、コミット対象にしません。
...
`cargo check` 時に `crates/bmz-app/src/app.rs` の `SelectChartRow` unused import warning が出ることがあります。今回の作業に関係しない場合は無理に触らないでください。
