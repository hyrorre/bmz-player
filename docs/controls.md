...
UI倍率はプルダウンで100・125・150・175・200%から選択できます。OSの表示倍率に追加して適用し、
BMZメニュー・設定・Practiceなどeguiの補助UI全体を拡大します。プレイスキンには影響しません。
変更は即時反映され、プロファイルに自動保存されます。既存プロファイルの既定値は100%です。
...
選曲画面では、設定 → プロファイル → プロファイル管理で切り替え先を選び、
「切り替える」を押すと再起動なしでプロファイルを変更できます。新規作成・複製時も
「作成後に切り替える」を選べます。現在の編集を保存し、切り替え先の設定・スコア・
お気に入り・IR・スキンを読み直して選曲ルートへ戻ります。切り替え後は通常プレイとなり、
選曲の絞り込み・並び順は切り替え先の保存値を使います。成功したプロファイルを次回起動にも使います。
Decide / Play / Result、Viewer、取り込み・ログイン・コース編集などの処理中は切り替えできません。
準備中は現在の選曲画面を表示したまま操作を一時停止し、切り替え先の選曲スキンの準備と
IR送信中の1件の完了を待って、プロファイルとスキンをまとめて切り替えます。スキンを含む読み込み・保存に失敗した場合は
現在のプロファイルを維持します。`--profile` での起動だけでは次回起動設定を変更しません。

スキン設定の「追加スキンのフォルダーを開く」は、現在のデータ保存先の `skins` を開きます。
スキンを配置したら「スキン一覧を更新」で再取得し、画面ごとに選択してください。
選択先の実パスと読み込みエラーもスキン設定で確認できます。エラー時は保存済みの選択・オプションを保持します。
...
Decide / Play中も一般・音声・映像・連携・録画を編集・保存できます。
音声出力の「適用」は選曲画面へ戻るまで保留します。ASIOデバイスの更新は演奏中は無効です。
OBS設定をDecide / Play / Result中に変更した場合は、その後の変更も含め次回起動時に反映します。
現在の録画の停止待ち・保存判定を保持するため、今回の起動では変更前の設定を使い続けます。
その他の項目は従来の変更制限を維持し、ページ移動・閲覧は可能です。
...
スクリーンショットのクリップボードコピーが有効な場合は、PNG保存後に画像もコピーします。
Linux の Wayland（Flatpak を含む）では、BMZ のウィンドウにフォーカスがある状態でコピーしてください。
...
`.bms` / `.bme` / `.bml` / `.pms` / `.bmson`ファイルをSelect画面へドラッグ＆
ドロップすると、その譜面をライブラリへ取り込んで通常プレイを開始します。
通常起動ではSelect以外の画面へのドロップは受け付けません。Viewerウィンドウへのドロップは、
再生中・一時停止中・待機中とも、その譜面をViewerモードで0小節目から再生します。
Battle ViewerではBattle設定を維持します。外部BMSエディタのビューワー設定は
...
F1設定の「ライブラリ再スキャン」は選曲位置にかかわらず、設定を保存してから全ルートを再解析します。
設定から削除したルートの登録も整理します（実ファイル・スコア・リプレイは削除しません）。
無効化したルートや一時的に読めない設定内ルートの登録は保持します。
F5と個別ルートの再読み込みは対象フォルダだけを更新します。
CLIの対象無指定の `songs load` / `songs reload` も全体同期です。
`songs load <PATH>` は部分更新です。設定外の場所も次回の全体同期までは再起動後も利用でき、設定ファイルには追加しません。無効に設定された場所は部分更新後も選曲対象外です。全体同期後も保持する場所は `songs add <PATH>` で設定に登録してください。
...
難易度表のレベルフォルダ内では、BMZの「レベル表示」設定をスキンのレベル欄にも適用します。
「難易度表」は現在の表のレベル、「譜面」は譜面本来のレベルを表示します。未所持などで
...
取得に成功した保存先は、有効な再帰スキャン対象の曲フォルダとして設定にも保存します。
有効な再帰スキャン対象の親フォルダが登録済みなら重複追加しません。
保存先自身を無効にしていた場合は、再度取得すると有効になります。
...
譜面の読み込みに失敗した場合は、決定画面・プレイ開始待ちのどちらでも開始を中止し、
選曲画面へ戻って対象パスを含むエラーを通知します。
