...
Selectのoption `100..105` / `1100..1104` は選択中スコアのクリア状態を参照する。
Luaのロード時にもoption `50/51` とtext `1020`へprimary IRの接続設定を渡すため、
接続の有無で生成するimageset等を切り替えるスキンも利用できる。

IRのクリア種別人数・割合（number `202..219`, `222..242`）は、全件取得が確認できる
ランキングから計算する。フルコンボ人数はFULLCOMBO/PERFECT/MAXの合計で、割合は
整数部と小数第1位を提供する。一部の順位だけ取得した場合、人数や小数部分は非表示。
提供元の整数クリア率がある場合は `227` に表示し、人数への逆算は行わない。

Selectのnumber `350/351` は鍵盤の通常ノーツ/LN数で、スクラッチ/BSS (`352/353`) を
