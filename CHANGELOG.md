...
- 外部BMSエディタ向けの Viewer モードを追加しました。
  ...
  - 再生終了後も待機し、後続の再生要求を同じウィンドウへ転送します。一時停止・再開、シーク、譜面のドラッグ＆ドロップによる切り替えにも対応しました。
...
- プレイ動画のオフライン書き出しを追加しました。
  ...
  - 音声デバイスとウィンドウは不要です。GPUと外部FFmpegを使用し、スコア保存・IR送信は行いません。

- CLIから起動時の設定を一時的に変更できるようにしました。
  - 配置、ゲージ、GAUGE AUTO SHIFT、ハイスピード、BGA、オートスクラッチ、画面モード・サイズ・モニターを指定できます。設定ファイルには保存しません。
  - `monitors list` によるモニター一覧と、`--print-effective-options` による実効設定の確認を追加しました。Viewerへの再生要求と動画出力でも共通のプレイ設定を利用できます。
...
- 従来の直接倍率方式（Classic）とFloatingに加え、20段階のNormalハイスピードと5種類のHS設定を追加しました。レーンカバーの有効設定、カバーを考慮した選曲中の表示時間・緑数字も反映します。
- 未処理の通常ノートを判定確定まで判定ラインへ留める表示設定を追加しました。既定では無効です。
- キー音の自動再生を追加し、打鍵音との二重再生を防ぎつつ、除外レーン・Mine・代替キー音の扱いを設定できるようにしました。
- IR接続先としてBMS-IRを追加しました。
- BMS-IR のコース連携を拡張し、LR2互換のコースキーとコース中の単曲結果送信に対応しました。難易度表の `charts`・平坦なMD5配列・旧 `grade` 形式のコースも取り込めるようにしました。
...
- LITONE11、動的なLuaランク値、数値形式の `isRefNum`、テキスト縮小、2体目のPMchara、E1 / E2パネルタイマーの互換性を改善しました。
- LR2 Battle のゲージ・スコア表示と、プロファイルのBGA拡縮設定の継承を修正しました。ノーツの透明度オフセット、次ランク差分、ライバル名、選曲スキンの譜面レベル表示も修正しました。
