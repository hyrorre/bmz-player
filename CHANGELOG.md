...
- 起動時の待ち時間を短縮しました。
  - 描画と設定UIでフォントの探索結果を共有し、同梱フォントを優先して不足する文字だけOSフォントで補うようにしました。
  - スキン一覧の走査と起動時のスキン読み込みを並行して進めるようにしました。

- 選曲画面でフォルダを開く際の負荷を軽減しました。
  - 譜面のコピーと解析情報をまとめて取得し、一覧表示にはスキャン済みの登録情報を使用することで、譜面ごとのファイル確認を省きました。再生開始時には実ファイルとハッシュを確認します。
  - 表示対象の密度グラフ・BPMグラフをバックグラウンドで取得し、再訪時にはキャッシュを利用するようにしました。
  - お気に入り情報を一覧間で再利用し、DB更新やプロファイル切り替えに合わせて更新するようにしました。

- Windows実行ファイルにアプリアイコンを埋め込み、エクスプローラーでも表示されるようにしました。

### 修正

- 全体再スキャンで古い曲フォルダの登録が残る問題と、設定画面からのスキャン範囲が選曲位置に依存する問題を修正しました。無効・切断中の曲フォルダや単体取り込みの登録は保持します。
- 設定外のフォルダを個別スキャンした直後から再生対象外になる問題を修正しました。個別スキャン範囲は再起動後も利用でき、次の全体同期で整理します。設定済みの曲フォルダの無効指定は優先します。
- 同じ譜面が複数の場所にある場合、古い登録や無効な曲フォルダが選ばれ、プレイ・プレビュー・難易度表の所持判定が失敗する問題を修正しました。プレイ・練習・リプレイ・コースでは利用可能なコピーを選び、ID・フォルダ・メタデータを揃えます。
- 譜面PATHを指定して起動した際、編集前の別コピーが再生される問題を修正しました。指定ファイルの現在の内容をライブラリへ同期し、曲フォルダが無効でも明示したPATHからは再生できます。
- 曲フォルダの別名と正規パスの対応を保存し、無効な曲フォルダ内の譜面がPATH指定での起動後に通常の選曲候補へ混入する問題を修正しました。別名が利用できなくなった場合や再起動後も所属を維持します。
- コース開始前に利用可能な譜面コピーを解決し、参照先の更新後も代替コピーからプレイできるようにしました。途中までのコースリプレイは、未再生ステージのファイルが欠けていても再生できます。
- 部分コースリプレイの開始前に未プレイ曲への参照が失われる問題を修正しました。ファイルが欠けていても保存済みメタデータを使い、コース全体のノーツ数やスコア参照を保持します。
- LR2orajaのCN / HCNスコア取り込みが古い譜面パスの読み込みで失敗する問題を修正しました。保存済みのLN種別別ノーツ数で検証し、再スキャンが必要な場合や取り込めない理由を件数とともに表示します。
- Windows portable版の初回起動でインストーラー版のデータを参照する問題を修正しました。既定の保存先を実行ファイル横の `data` に固定し、明示的な保存先指定は維持します。
- Windowsの更新プログラム・メタデータ・作業ファイルを `updater` 配下へまとめ、旧配置からの更新・復旧にも対応しました。
- macOSの自動更新後、再起動への引き継ぎ中にクラッシュする問題を修正しました。
- macOSで更新確認が「更新なし」で正常終了してもエラー通知へ上書きされる問題を修正しました。認証キャンセルと区別し、失敗通知の重複も防ぎます。
- 更新ダウンロードのキャンセルや失敗で一時ファイルが残る問題と、更新ダイアログのキャンセルボタンに翻訳キーが表示される問題を修正しました。
- ループ動画の最終フレームが早く切り上がり、周回ごとに再生時刻がずれる問題を修正しました。通常のループと再生位置の巻き戻しも区別するようにしました。
- Windowsで同梱TTCフォントの地域別字形や文字幅が誤る問題を修正しました。設定UIの日本語が下へずれる問題も、フォントの字形に基づく位置調整で修正しました。

### テスト・開発環境

- 起動時間と選曲一覧の処理時間を計測し、フォント探索・スキン準備・譜面コピー解決の改善結果を記録しました。
- ライブラリ再スキャン、代替コピー・明示PATH起動、部分コースリプレイ、スコア取り込み、非同期グラフ・お気に入りキャッシュ、フォント表示、動画ループ、更新のキャンセル・再起動の回帰テストを追加・更新しました。
- 個別スキャン範囲と曲フォルダの別名の永続化、部分コースリプレイのメタデータ保持、macOSの更新確認結果の回帰テストを追加しました。
- macOS専用のSparkle検証exampleがWindows / Linuxのビルドと配布検証を妨げる問題を修正しました。
