...
6. Do not put secrets, service role keys, DB passwords, refresh tokens, or production data in repo files.
7. Ask explicit user approval before `bun run db:push`, destructive SQL, remote writes, or anything that touches production.
8. After migration changes, run:
   - `bun run db:reset`
   - `bun run db:types`
   - `bun run build`
9. If local Supabase is not running, explain that `bun run db:start` is required before local reset/type generation.
